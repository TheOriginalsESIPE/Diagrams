...
public ViewAuthentification() {
		...
		System.out.println("Auth Client.view");

}

	/**
	 * This is a Client.view to show to Client.client if there is an error.
	 * If it can't connect to the Server.server, Client.client terminal will dispose.
	 * @param errorType 1:The Client.client can't connect to the Server.server. 2: username or password incorrect.
     */
	public void errorDialog(int errorType){
		if(errorType==1) {
			JOptionPane.showMessageDialog(null, "Can't not connect to the Server." +
					"\nPlease check if the Server.server is launched and Server.properties files.");
			myFrame.dispose();
		} else if(errorType == 2)
